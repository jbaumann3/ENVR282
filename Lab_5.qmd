...
welch_anova_test(mpg~cyl, data=mtcars) #here, we do get a p-value that is <0.05, suggesting that there is an effect of cylinders on mpg
```
...
When building an ANOVA (or any statistical model) there are two ways to look at multiple factors. We can do additive (+) or interactive (\*). Additive tests the effects of each variable separately. Interactive does the same plus an interaction term.
...
To test that, we need to do a post-hoc test. I like the TUKEY honest significant difference test (which functions like a bunch of t-tests all at once)
...
\
**4.)** Read in [These data](https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2024/2024-03-05/trashwheel.csv) about trash cleanup in Baltimore Harbor. ![Mr. Trash Wheel doing his job](https://upload.wikimedia.org/wikipedia/commons/2/20/Mr._Trash_Wheel_%2827582099121%29.jpg)
